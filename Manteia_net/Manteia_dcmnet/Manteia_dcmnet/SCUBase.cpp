...
CSCUBase::CSCUBase() :
	...
	m_acseTimeout(30),
	m_storageDir(),
	m_storageMode(CSCUBase_STORAGE_DISK),
...
CSCUBase::~CSCUBase()
{
	...
	if (isConnected())
	{
		closeAssociation(CSCUBase_ABORT_ASSOCIATION); // also frees network
	}
	else {
		freeNetwork();
	}
...
T_ASC_Association* CSCUBase::GetT_ASC_Association()
{
	if (m_assoc)
	{
		return m_assoc;
...
//此函数只用于C-GET!!!!!
void CSCUBase::SetT_ASC_Association(T_ASC_Association * ass)
{
	if (ass == NULL)
	{
		return;
	...
	//此处的释放，只释放m_assoc的内存，不释放param和DULassociation的内存，因为C-GET调用SetT_ASC_Association
	//该函数时，只新建m_assoc的内存！！！！
	//if (m_assoc)
	//{
	//	ASC_destroyAssociation(&m_assoc);
	...
	//深拷贝！！！！！(由于有未知类型，无法分配内存，此处就直接使用浅拷贝！！！) //用完后，直接手动设置m_assoc = NULL！！
	//m_assoc = new T_ASC_Association();
	//m_assoc->nextMsgID = ass->nextMsgID;
	//m_assoc->DULassociation = ass->DULassociation;
	//m_assoc->params = ass->params;
	//m_assoc->sendPDVBuffer = ass->sendPDVBuffer;
	//m_assoc->sendPDVLength = ass->sendPDVLength;
	m_assoc = ass;
}

T_ASC_Network * CSCUBase::GetT_ASC_Network()
{
	if (m_net)
	{
		return m_net;
...
T_ASC_Parameters * CSCUBase::GetT_ASC_Parameters()
{
	if (m_params)
	{
		return m_params;
...
OFCondition CSCUBase::initNetwork()
{
	...
	OFListIterator(CSCUBasePresContext) contIt = m_presContexts.begin();
	OFListConstIterator(CSCUBasePresContext) endOfContList = m_presContexts.end();
...
OFCondition CSCUBase::negotiateAssociation()
{
...
OFCondition CSCUBase::addPresentationContext(const OFString &abstractSyntax,
	const OFList<OFString> &xferSyntaxes,
	const T_ASC_SC_ROLE role)

{

	CSCUBasePresContext presContext;
...
OFCondition CSCUBase::useSecureConnection(DcmTransportLayer *tlayer)
{
...
void CSCUBase::clearPresentationContexts()
{
...
T_ASC_PresentationContextID CSCUBase::findPresentationContextID(const OFString &abstractSyntax,
...
// transfer syntax UID.
T_ASC_PresentationContextID CSCUBase::findAnyPresentationContextID(const OFString &abstractSyntax,
	const OFString &transferSyntax)
{
	if (m_assoc == NULL)
		return 0;
...
void CSCUBase::findPresentationContext(const T_ASC_PresentationContextID presID,
...
void CSCUBase::closeAssociation(const MyCloseAssociationType closeType)
{
	if (!isConnected())
	{
	...
	switch (closeType)
	{
	case CSCUBase_RELEASE_ASSOCIATION:
		/* release association */
		...
		break;
	case CSCUBase_ABORT_ASSOCIATION:
		/* abort association */
		...
		break;
	case CSCUBase_PEER_REQUESTED_RELEASE:
		...
		break;
	case CSCUBase_PEER_ABORTED_ASSOCIATION:
...
OFCondition CSCUBase::releaseAssociation()
{
...
OFCondition CSCUBase::abortAssociation()
{
	OFCondition status = DIMSE_ILLEGALASSOCIATION;
	// check whether there is an active association
	if (isConnected())
	{
		closeAssociation(CSCUBase_ABORT_ASSOCIATION);
		status = EC_Normal;
	}
	return status;
}
...
OFCondition CSCUBase::sendECHORequest(const T_ASC_PresentationContextID presID)
{
	if (!isConnected())
...
OFCondition CSCUBase::sendSTORERequest(const T_ASC_PresentationContextID presID,
...
OFCondition CSCUBase::sendMOVERequest(const T_ASC_PresentationContextID presID,
...
OFCondition CSCUBase::handleMOVEResponse(const T_ASC_PresentationContextID /* presID */,
...
OFCondition CSCUBase::sendCGETRequest(const T_ASC_PresentationContextID presID,
	DcmDataset *dataset,
	OFList<RetrieveResponse*> *responses)
{
	// Do some basic validity checks
...
OFCondition CSCUBase::handleCGETSession(const T_ASC_PresentationContextID /* presID */,
	DcmDataset * /* dataset */,
	OFList<RetrieveResponse*> *responses)
{
	OFCondition result;
	...
	while (continueSession)
	{
		...
		if (rsp.CommandField == DIMSE_C_GET_RSP)
		{
			...
			//if (getRSP->m_status == STATUS_Success)
			//{
			//	//接收剩余数据(但此数据无用，只是收尾数据)
			//	// Check if dataset is announced correctly
		...
		else if (rsp.CommandField == DIMSE_C_STORE_RQ)
		{
			...
			if (m_storageMode == CSCUBase_STORAGE_DISK)
			{
				// Receive dataset
				result = receiveDIMSEDataset(&pcid, &rspDataset);
				if (result.bad())
			...
			else if (m_storageMode == CSCUBase_STORAGE_BIT_PRESERVING)
			{
...
OFCondition CSCUBase::handleCGETResponse(const T_ASC_PresentationContextID /* presID */,
	RetrieveResponse* response,
...
OFCondition CSCUBase::handleSTORERequest(const T_ASC_PresentationContextID /* presID */,
...
OFCondition CSCUBase::handleSTORERequestFile(T_ASC_PresentationContextID *presID,
...
OFCondition CSCUBase::sendSTOREResponse(T_ASC_PresentationContextID presID,
	Uint16 status,
...
OFString CSCUBase::createStorageFilename(DcmDataset *dataset)
{
...
OFCondition CSCUBase::ignoreSTORERequest(T_ASC_PresentationContextID presID,
	const T_DIMSE_C_StoreRQ &request)
{
...
void CSCUBase::notifyInstanceStored(const OFString &filename,
...
OFCondition CSCUBase::sendFINDRequest(const T_ASC_PresentationContextID presID,
	...
	OFList<QRResponse*> *responses)
{
	...
	while (waitForNextResponse)
	{
		...
		//else if (findRSP->m_status == STATUS_Success)
		//{
		//	//接收剩余数据(但此数据无用，只是收尾数据)
		//	// Check if dataset is announced correctly
		//	if (rsp.msg.CFindRSP.DataSetType == DIMSE_DATASET_NULL)
...
OFCondition CSCUBase::handleFINDResponse(const T_ASC_PresentationContextID /* presID */,
	QRResponse *response,
	OFBool &waitForNextResponse)
{
	if (!isConnected())
...
OFCondition CSCUBase::sendCANCELRequest(const T_ASC_PresentationContextID presID)
{
	if (!isConnected())
		return DIMSE_ILLEGALASSOCIATION;
...
OFCondition CSCUBase::sendACTIONRequest(const T_ASC_PresentationContextID presID,
...
OFCondition CSCUBase::sendEVENTREPORTRequest(const T_ASC_PresentationContextID presID,
	const OFString &sopInstanceUID,
...
OFCondition CSCUBase::handleEVENTREPORTRequest(DcmDataset *&reqDataset,
	Uint16 &eventTypeID,
	const int timeout)
{
	// Do some basic validity checks
...
Uint16 CSCUBase::checkEVENTREPORTRequest(T_DIMSE_N_EventReportRQ & /*eventReportReq*/,
	DcmDataset * /*reqDataset*/)
{
...
void CSCUBase::notifySENDProgress(const unsigned long byteCount)
{
	DCMNET_TRACE("Bytes sent: " << byteCount);
}


void CSCUBase::notifyRECEIVEProgress(const unsigned long byteCount)
{
...
OFCondition CSCUBase::sendDIMSEMessage(const T_ASC_PresentationContextID presID,
	T_DIMSE_Message *msg,
...
OFCondition CSCUBase::receiveDIMSECommand(T_ASC_PresentationContextID *presID,
	T_DIMSE_Message *msg,
...
OFCondition CSCUBase::receiveDIMSEDataset(T_ASC_PresentationContextID *presID,
	DcmDataset **dataObject)
{
	if (!isConnected())
		return DIMSE_ILLEGALASSOCIATION;
...
void CSCUBase::setMaxReceivePDULength(const Uint32 maxRecPDU)
{
	m_maxReceivePDULength = maxRecPDU;
}


void CSCUBase::setDIMSEBlockingMode(const T_DIMSE_BlockingMode blockingMode)
{
	m_blockMode = blockingMode;
}


void CSCUBase::setAETitle(const OFString &myAETtitle)
{
	m_ourAETitle = myAETtitle;
}


void CSCUBase::setPeerHostName(const OFString &peerHostName)
{
	m_peer = peerHostName;
}


void CSCUBase::setPeerAETitle(const OFString &peerAETitle)
{
	m_peerAETitle = peerAETitle;
}


void CSCUBase::setPeerPort(const Uint16 peerPort)
{
	m_peerPort = peerPort;
}


void CSCUBase::setDIMSETimeout(const Uint32 dimseTimeout)
{
	m_dimseTimeout = dimseTimeout;
}


void CSCUBase::setACSETimeout(const Uint32 acseTimeout)
{
	m_acseTimeout = acseTimeout;
}


void CSCUBase::setConnectionTimeout(const Sint32 connectionTimeout)
{
	dcmConnectionTimeout.set(connectionTimeout);
}


void CSCUBase::setAssocConfigFileAndProfile(const OFString &filename,
...
void CSCUBase::setStorageDir(const OFString& storeDir)
{
	m_storageDir = storeDir;
}


void CSCUBase::setStorageMode(const MyStorageMode storageMode)
{
	m_storageMode = storageMode;
}


void CSCUBase::setVerbosePCMode(const OFBool mode)
{
	m_verbosePCMode = mode;
}


void CSCUBase::setDatasetConversionMode(const OFBool mode)
{
	m_datasetConversionMode = mode;
}


void CSCUBase::setProgressNotificationMode(const OFBool mode)
{
...
OFBool CSCUBase::isConnected() const
{
	return (m_assoc != NULL) && (m_assoc->DULassociation != NULL);
}

Uint32 CSCUBase::getMaxReceivePDULength() const
{
	return m_maxReceivePDULength;
}


OFBool CSCUBase::getTLSEnabled() const
{
	return OFFalse;
}


T_DIMSE_BlockingMode CSCUBase::getDIMSEBlockingMode() const
{
	return m_blockMode;
}


const OFString &CSCUBase::getAETitle() const
{
	return m_ourAETitle;
}


const OFString &CSCUBase::getPeerHostName() const
{
	return m_peer;
}


const OFString &CSCUBase::getPeerAETitle() const
{
	return m_peerAETitle;
}


Uint16 CSCUBase::getPeerPort() const
{
	return m_peerPort;
}


Uint32 CSCUBase::getDIMSETimeout() const
{
	return m_dimseTimeout;
}


Uint32 CSCUBase::getACSETimeout() const
{
	return m_acseTimeout;
}


Sint32 CSCUBase::getConnectionTimeout() const
{
	return dcmConnectionTimeout.get();
}


OFString CSCUBase::getStorageDir() const
{
	return m_storageDir;
}


MyStorageMode CSCUBase::getStorageMode() const
{
	return m_storageMode;
}


OFBool CSCUBase::getVerbosePCMode() const
{
	return m_verbosePCMode;
}


OFBool CSCUBase::getDatasetConversionMode() const
{
	return m_datasetConversionMode;
}


OFBool CSCUBase::getProgressNotificationMode() const
{
	return m_progressNotificationMode;
}


OFCondition CSCUBase::getDatasetInfo(DcmDataset *dataset,
	OFString &sopClassUID,
	OFString &sopInstanceUID,
...
void CSCUBase::callbackSENDProgress(void *callbackContext,
	const unsigned long byteCount)
{
	if (callbackContext != NULL)
		OFreinterpret_cast(CSCUBase *, callbackContext)->notifySENDProgress(byteCount);
}


void CSCUBase::callbackRECEIVEProgress(void *callbackContext,
	const unsigned long byteCount)
{
	if (callbackContext != NULL)
		OFreinterpret_cast(CSCUBase *, callbackContext)->notifyRECEIVEProgress(byteCount);
}

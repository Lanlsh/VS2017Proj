...
class MANTIA_DECL_EXPORT CSCPBase
{
public:
	CSCPBase();
	~CSCPBase();
	 ...
	 *          <li>EC_setuidFailed: Returned (on Unix-like systems) if the CSCPBase
	 ...
	 *  class is added here, CSCPBase will respond to related C-ECHO requests. Note
	 *  that this cannot be reverted.
	 *  The default behavior of CSCPBase is not to support any SOP Class at all.
	 *  @param profile [in] The profile Verification SOP Class should
	 *                 be added to. The default is to add it to the
	 *                 CSCPBase's internal standard profile called
	 *                 "DEFAULT".
	 ...
	 *                        is allowed to negotiate the SCP role, thus, that this CSCPBase instance
	 *                        will be playing the SCU role for this abstract syntax. The default
	 *                        role (ASC_SC_ROLE_DEFAULT) implicates that this CSCPBase instance
	 ...
	 *                             default is to add it to the CSCPBase's internal standard
	 ...
	 *                        for responding. CSCPBase's default is OFFalse.
	 */
	 ...
	 *  offering
	 *  @param doRefuse [in] If OFTrue, every association is being refused. CSCPBase's default
	 ...
	 *  returns. The default of CSCPBase is blocking mode.
	 ...
	 *  on the configuration object is shadowed by other API functions of CSCPBase.
	 *  The existing functions are provided in order to not break users of this
	 *  "older" API where no configuration object existed.
	 *  @return a reference to the DcmSCPConfig object used by this CSCPBase object.
	 */
	virtual DcmSCPConfig& getConfig();

	/** Set the DcmSCPConfig object to use for configuring this CSCPBase object.
	...
	//get the m_assoc instance
	T_ASC_Association* GetAssociationInstance();
...
protected:
	 ...
	 *          CSCPBase only handles C-ECHO requests and, therefore, returns
...
private:
	...
	/// it, e.g. in the context of the CSCPBasePool class.
	 ...
	 */
	CSCPBase(const CSCPBase &src);

	/** Private undefined assignment operator. Shall never be called.
	 *  @param src Source object
	 *  @return Reference to this
	 */
	CSCPBase &operator=(const CSCPBase &src);

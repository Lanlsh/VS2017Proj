...
/*
	此类只用于SCP接收到C-GET时，SCP发送C-STORE！！！
*/
class MANTIA_DECL_EXPORT CStoreSCUForCGet: public CSCUBase
{
public:
	CStoreSCUForCGet();
	~CStoreSCUForCGet();
...
private:
	...
	// private undefined copy constructor
	CStoreSCUForCGet(const CStoreSCUForCGet &);

	// private undefined assignment operator
	CStoreSCUForCGet &operator=(const CStoreSCUForCGet &);

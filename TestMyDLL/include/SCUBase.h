 ...
 */
enum DCMTK_DCMNET_EXPORT MyCloseAssociationType
{
	/// Release the current association
	CSCUBase_RELEASE_ASSOCIATION,
	/// Abort the current association
	CSCUBase_ABORT_ASSOCIATION,
	/// Peer requested release (Aborting)
	CSCUBase_PEER_REQUESTED_RELEASE,
	/// Peer aborted the association
	CSCUBase_PEER_ABORTED_ASSOCIATION
};
...
enum MyStorageMode
{
	...
	CSCUBase_STORAGE_IGNORE,
	/// Try to store the objects to disk
	CSCUBase_STORAGE_DISK,
	...
	CSCUBase_STORAGE_BIT_PRESERVING
};
 ...
 *  on that connection. CSCUBase has built-in C-ECHO support so derived classes do not have to
 *  implement that capability on their own.
 *  @warning This class is EXPERIMENTAL. Be careful to use it in production environment.
 */
class MANTIA_DECL_EXPORT CSCUBase
{
public:
	 ...
	 */
	virtual void closeAssociation(const MyCloseAssociationType closeType);
	...
	/** Set the storage mode to be used. Default is CSCUBase_STORAGE_DISK.
	 *  @param storageMode The storage mode to be used.
	 */
	void setStorageMode(const MyStorageMode storageMode);
	 ...
	 */
	MyStorageMode getStorageMode() const;
	...
	/** Get the Association of this SCU*/
	T_ASC_Association* GetT_ASC_Association();

	/** Set the Association of this SCU*/
	//It is only can be used for C_GET
	void SetT_ASC_Association(T_ASC_Association* ass);

	/// Get The DICOM network the association is based on
	T_ASC_Network* GetT_ASC_Network();

	/// Get Association parameters
	T_ASC_Parameters* GetT_ASC_Parameters();
...
protected:
	...
	/** Tells CSCUBase to use a secure TLS connection described by the given TLS layer
	 ...
	 * *********************************************************************** */
	  ...
	  *  the SCU is in storage mode CSCUBase_STORAGE_IGNORE.
	 ...
	 *  function is only called if the SCU is in CSCUBase_STORAGE_DISK mode.
...
private:
	 ...
	 */
	CSCUBase(const CSCUBase &src);

	/** Private undefined operator=. Shall never be called.
	 *  @param src Source object
	 *  @return Reference to this
	 */
	CSCUBase &operator=(const CSCUBase &src);
	...
	OFList<CSCUBasePresContext> m_presContexts;
	...
	/// Default value: CSCUBase_STORAGE_DISK
	MyStorageMode m_storageMode;
